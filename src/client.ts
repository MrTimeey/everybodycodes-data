...
export class EverybodyCodesClient {
    ...
    private makeUrl(type: ChallengeType, ...parts: (string | number)[]): string {
        return [API_BASE, type, ...parts.map(p => encodeURIComponent(String(p)))].join("/");
    }
    ...
    private async getSeed(): Promise<string> {
        ...
        const data = await http<{ seed?: string }>(url, { headers: this.cookieHeader() });
        if (!data?.seed) throw new Error("Kein 'seed' in /user/me Antwort.");
        this.seed = data.seed;
        return data.seed;
    }

    private async getEncryptedInputs(event: string | number, quest: string | number, seed: string) {
        const url = `${CDN_BASE}/${encodeURIComponent(String(event))}/${encodeURIComponent(
            String(quest)
        )}/input/${encodeURIComponent(seed)}.json`;
        return await http<Record<"1" | "2" | "3", string>>(url, { headers: this.cookieHeader() });
    }

    private async getDataInternal(type: ChallengeType, event: string | number, quest: string | number) {
        ...
        (["1", "2", "3"] as const).forEach(p => {
            ...
            const key = keys[p];
            if (hex && key) out[Number(p) as 1 | 2 | 3] = aesDecryptHexWithKey(key, hex);
        });
        return out;
    }


    private async getKeys(type: ChallengeType, event: string | number, quest: string | number) {
     ...
     */
    async getEventPartData(event: string | number, quest: string | number, part: PartNumber) {
     ...
     */
    async getStoryPartData(story: string | number, quest: string | number, part: PartNumber) {
